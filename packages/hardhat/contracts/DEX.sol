...
contract DEX {
    ...
    /**
     * @notice Emitted when when spender contract is approved for transaction
     */
    ...
    function tokenToEth(uint256 tokens) public returns (uint256) {
        ...
        require(token.transferFrom(msg.sender, address(this), tokens));
        emit TokenToEthSwap(msg.sender, "Balloons to Eth", token_reserve, eth_bought);
        return eth_bought;
    }
    ...
    function withdraw(uint256 amount) public returns (uint256, uint256) {
        ...
        require(sent, "withdraw: revert in wathdrawing eth to you!");
        require(token.transfer(msg.sender, token_amount));
        emit LiquidityRemoved(msg.sender, token_amount, eth_amount, totalLiquidity);
